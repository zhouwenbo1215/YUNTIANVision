    ...
    public partial class frmTwoCamera : Form
    {
        ...
        private void runGrabOneCameraThread()
        {
            ...
            //{
                while (isRunning)
                {
                    ...
                    if (DeepLearning.res1.Count!=0 && DeepLearning.res1[treeFileHelper1.dingweiNodeId].Count == 1)
                    {
        ...
        private void rungrabTwoCameraThread()
        {
            ...
            //{
                while (isRunning)
                {
                    ...
                    HTCSharpDemo.Program.ImageHt imageHt = new HTCSharpDemo.Program.ImageHt();
                MyCamera.MV_FRAME_OUT stFrameOut = new MyCamera.MV_FRAME_OUT();
                getImgAndDisply(device2, ref hWindowControl2, out ho_image2, ref imageHt,ref stFrameOut);
                    ...
                    if (DeepLearning.res2.Count != 0 && DeepLearning.res2[treeFileHelper2.dingweiNodeId].Count == 1)
                    {
                ...
                device2.MV_CC_FreeImageBuffer_NET(ref stFrameOut);
                // 通知plc线程完成
                countdown.Signal();
                }
            //}
        ...
        private  void openOneHikVisionCam()
        {
            try
            {
                ...
                else tbLog1.BeginInvoke(GetMyDelegateLog, tbLog1, "相机1初始化失败...");
             ...
             try
                {
                    ...
                    else tbLog2.BeginInvoke(GetMyDelegateLog, tbLog2, "相机2初始化失败...");
        ...
        private void getImgAndDisply(MyCamera device,ref HWindowControl hWindowControl, out HObject image,ref HTCSharpDemo.Program.ImageHt imageHt,ref MyCamera.MV_FRAME_OUT stFrameOut)
        {
            ...
            if (MyCamera.MV_OK == nRet)
            {
                ...
                HalconHelper.imageLocation(stFrameOut.stFrameInfo.nWidth, stFrameOut.stFrameInfo.nHeight, hWindowControl.Width, hWindowControl.Height, out row3, out column3, out row4, out column4);
                HOperatorSet.SetPart(hWindowControl.HalconWindow, row3, column3, row4, column4);
                HalconDisplay(hWindowControl.HalconWindow, image, row3,column3, row4, column4);
       ...
       // 委托显示通讯状态
        ...
        private async void showNGLocation1(Dictionary<int, List<result>> res1)
        {
            await Task.Run(() =>
            {
                ...
                {
                    if (res1.Count !=0 && res1[treeFileHelper1.dingweiNodeId].Count == 1)
                    {
                        HTCSharpDemo.Program.result res2 = res1[treeFileHelper1.dingweiNodeId][0];
        ...
        private async void showNGLocation2(Dictionary<int, List<result>> res3)
        {
            await Task.Run(() =>
            {
                ...
                {
                    if (res3.Count !=0 && res3[treeFileHelper2.dingweiNodeId].Count == 1)
                    {
                        HTCSharpDemo.Program.result res2 = res3[treeFileHelper2.dingweiNodeId][0];
        ...
        #region 更新结果列表
        private async void referenceTable1(Dictionary<int, List<result>> res1)
        {
        ...
        private async void referenceTable2(Dictionary<int, List<result>> res2)
        {
            await Task.Run(new Action(() =>
            {
                string configPath = IniHelper.SaveSetIni.Read("深度学习文件", "配置文件路径");
                        ...
                        if (dataGridView4.Rows.Count - 1 > row.Index)
                        {
                            ...
                            if (row.Cells[0].Value != null && !String.IsNullOrWhiteSpace(row.Cells[0].Value.ToString()) && row.Cells[0].Value != DBNull.Value)
                            {
                                if ((string)row.Cells[1].Value == "定位失败")
                                {
                                    ...
                                    if (row.Cells[2].Value != null && !String.IsNullOrWhiteSpace(row.Cells[2].Value.ToString()) && row.Cells[2].Value != DBNull.Value && Regex.IsMatch(row.Cells[2].Value.ToString(), @"^\d+(\.\d+)?$"))
                                        ngdingweiNum = Convert.ToInt32(row.Cells[2].Value);
                                    if (res2.Count != 0 && res2[treeFileHelper2.dingweiNodeId].Count == 1)
                                        row.Cells[3].Value = "--";
                                    else
                                    {
                                        row.Cells[2].Value = ngdingweiNum + 1;
                ...
                while (isRunning)
                 {
                    switch (camStatus)
                    {
                        case "相机1":
                            //grabOneImage(out ho_image1);
                            //oneImageToHalcon(ho_image1);
                            HTCSharpDemo.Program.ImageHt imageHt1 = new HTCSharpDemo.Program.ImageHt();
                            MyCamera.MV_FRAME_OUT stFrameOut = new MyCamera.MV_FRAME_OUT();
                            getImgAndDisply(device1, ref hWindowControl1, out ho_image2, ref imageHt1,ref stFrameOut);
                            device1.MV_CC_FreeImageBuffer_NET(ref stFrameOut); 
                            break;
                        case "相机2":
                            //grabTwoImage(out ho_image2);
                            //twoImageToHalcon(ho_image2);
                            HTCSharpDemo.Program.ImageHt imageHt2 = new HTCSharpDemo.Program.ImageHt();
                            MyCamera.MV_FRAME_OUT stFrameOut1 = new MyCamera.MV_FRAME_OUT();
                            getImgAndDisply(device2, ref hWindowControl2, out ho_image2, ref imageHt2,ref stFrameOut1);
                            device2.MV_CC_FreeImageBuffer_NET(ref stFrameOut1);
                            break;
                        default:
                            break;
                    }
