...
namespace YUNTIANVision
{
    ...
    public class CamerasHikvision : CamerasBase
    {
        ...
        /// 获取所有相机
        /// </summary>
        /// <returns>List<MyCamera.MV_CC_DEVICE_INFO></returns>
        public static List<MyCamera.MV_CC_DEVICE_INFO> getCameraInfos()
        {
            ...
            if (stDevList.nDeviceNum == 0)
                return new List<MyCamera.MV_CC_DEVICE_INFO>();
            List < MyCamera.MV_CC_DEVICE_INFO > stDevInfos = new List<MyCamera.MV_CC_DEVICE_INFO>();
            for (int i = 0; i < stDevList.nDeviceNum; i++)
            {
                ...
                stDevInfos.Add(stDevInfo);
            }
            return stDevInfos;
        }

        /// <summary>连接某个相机</summary>
        public override void ConnectDev(ref MyCamera.MV_CC_DEVICE_INFO CurDevice)
        {
            try
            {
                base.ConnectDev(ref CurDevice);
                // 如果设备已经连接先断开
                DisConnectDev();
                FindCBySN(mSerialNo);
                int nRet = -1;
