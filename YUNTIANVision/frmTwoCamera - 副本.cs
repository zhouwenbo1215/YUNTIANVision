...
namespace YUNTIANVision
{
    ...
    public partial class frmTwoCamera : Form
    {
                ...
                while (isRunning)
                {
                    if (m_bRunThread1)
                    {
                        ...
                        while (isRunning)
                        {
                            ...
                            switch (step)
                            {
                                case (int)STEP1.IMAGE_SIGNAL:
                                    tbLog1.BeginInvoke(GetMyDelegateLog, tbLog1, "相机1开始拍照...");
                                    break;
                                case (int)STEP1.GRAB_IMAGE:
                                    {
                                        Thread.Sleep(20);
                    ...
                    if (m_bRunThread2)
                    {
                        ...
                        while (isRunning)
                        {
                            ...
                            } else
                            {
                                ...
                                if (string.IsNullOrEmpty(tbGrabImageSignal2.Text))
                                {
        ...
        private void loadTwoPort()
        {
            try
            {
                ...
                else
                {
                    ...
                    btOpenPlc1.Enabled = true;
                    controlCom2(true);
                    showLog(tbLog2, $"{cbPortName2.Text}打开失败,当前使用相机1的COM口连接...");
        ...
        private async void ProcResult2(List<string> NGItems)
        {
            ...
            {
                string configPath = IniHelper.SaveSetIni.Read("深度学习文件", "配置文件路径");
                if(!panasonicMewtocol2.IsOpen()) //没有打开
                {
                    switch (result2)
                    {
                        case "OK":
                        ...
                        case "OK":
                            {
                                ...
                                tbLog1.BeginInvoke(GetMyDelegateLog, tbLog2, $"{tbOKSignal2.Text}信号次发送成功");
                                Thread.Sleep(200);
                                do
                                {
                                ...
                                tbLog1.BeginInvoke(GetMyDelegateLog, tbLog2, $"{tbOKSignal2.Text}信号次断开成功");
                                break;
                            }
                        case "NG":
                            {
